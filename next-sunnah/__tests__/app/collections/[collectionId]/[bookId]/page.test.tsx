...
describe("BookPage", () => {
  ...
  test("renders book information correctly", async () => {
    ...
    // Check if book Arabic name is rendered (now appears twice - once for desktop, once for mobile)
    const arabicNameElements = screen.getAllByText("اختبار الكتاب");
    expect(arabicNameElements.length).toBeGreaterThan(0);

    // Check if metadata is rendered (desktop view has combined text, mobile has separate)
    expect(screen.getByText("100 Hadiths • 10 Chapters")).toBeInTheDocument();
  ...
  test("renders chapters and hadiths correctly", async () => {
    ...
    const hadithLinks = screen
      .getAllByRole("link")
      .filter((link) => link.getAttribute("href")?.includes("/test-hadith-"));
    expect(hadithLinks[0]).toHaveAttribute(
      "href",
      "/collections/test-collection/test-book/test-hadith-1"
    );
    expect(hadithLinks[1]).toHaveAttribute(
      "href",
      "/collections/test-collection/test-book/test-hadith-2"
    );
  });
  ...
  test("renders empty state when no chapters are available", async () => {
    ...
    expect(
      screen.getByText("No chapters available for this book.")
    ).toBeInTheDocument();
  });
  ...
  test("renders empty state when no hadiths are available for a chapter", async () => {
    ...
    // Check if empty state message is rendered
    expect(
      screen.getByText("No hadiths available for this chapter.")
    ).toBeInTheDocument();
  });
});
